					...
					<table class="table table-condensed">
						...
						<tbody>
							...
							if(isset($_GET['buscar']) && $_GET['buscar']){
								$query = "SELECT * FROM tblPapel WHERE nome LIKE '%$_POST[busca]%'";
								...
								if ($result->num_rows > 0) {
									...
									while($row = $result->fetch_assoc()) {
										...
										if(isset($_SESSION['logado']) && $_SESSION['logado']){
											echo "<td><a type='button' href='/pratica5/tblPapel/edite.php?id=".$row["id"]."' class='btn btn-warning'>Edite</a></td>";
											echo "<td><a type='button'href='/pratica5/tblPapel/gravar.php?id=".$row["id"]."&deletar=true' class='btn btn-danger'>Apagar</a></td>";
										}
									}
								}
								else {
